...
#For ALL_AML_grow.train.noaffy.tmp the output file generated by this program is called "normalized_train.tmp"
#For ALL_AML_grow.test.noaffy.tmp the output file generated by this program is called "normalized_test.tmp"
#
use strict;
use warnings;
sub normalize{
    my ($input_filename, $output_filename) = @_;
    open(my $writeHandle, '>', $output_filename);
    open(my $readHandle, '<', $input_filename);
    my $ID_row = 1;
    while(my $row = <$readHandle>){
        my $current_line = "";
        ...
        print $writeHandle $current_line;        
    }
}
